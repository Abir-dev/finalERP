...
import { ArrowLeft, MapPin, User, Calendar, Clock, Truck, Wrench, AlertTriangle, CheckCircle, AlertCircle } from "lucide-react";
...
const MaintenanceVehiclesView: React.FC<MaintenanceVehiclesViewProps> = ({ onBack, totalMaintenanceCount, userId }) => {
  ...
  const getMaintenanceInfo = (vehicle: Vehicle) => {
    ...
    const maintenanceDuration = Math.ceil((today.getTime() - lastServiced.getTime()) / (1000 * 60 * 60 * 24));
    const daysUntilNext = Math.ceil((nextDue.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const getMaintenanceProgress = (maintenanceInfo: any) => {
    ...
    const progress = Math.min(100, (maintenanceInfo.maintenanceDuration / typicalMaintenanceDays) * 100);
    return progress;
  };
    ...
    <Card className="shadow-lg">
      <CardHeader className="flex flex-row items-center justify-between pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-xl">Vehicles Under Maintenance</CardTitle>
            <p className="text-sm text-muted-foreground">
              {maintenanceVehicles.length} vehicle{maintenanceVehicles.length !== 1 ? 's' : ''} currently in workshop
            </p>
          </div>
        </div>
      ...
      <CardContent className="space-y-4">
        {maintenanceVehicles.length === 0 ? (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No Vehicles Under Maintenance</h3>
            <p className="text-gray-500">All vehicles are currently operational or idle.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {maintenanceVehicles.map((vehicle) => {
              ...
              const priorityColors = {
                ...
                low: { border: 'border-l-blue-500', bg: 'bg-blue-50', text: 'text-blue-700', badge: 'bg-blue-100 text-blue-800' }
              };
                    ...
                    {maintenanceInfo && (
                      <div className={`${colors.bg} p-3 rounded-lg`}>
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <Badge variant="outline" className={`ml-auto text-xs ${colors.badge}`}>
                            {priority.toUpperCase()} PRIORITY
                          </Badge>
                        </div>
                        <p className={`text-sm ${colors.text} mb-2`}>
                          In maintenance for {maintenanceInfo.maintenanceDuration} day{maintenanceInfo.maintenanceDuration !== 1 ? 's' : ''}
                        </p>
                        <Progress value={progress} className="h-2 mb-2" />
                        <div className="text-xs text-gray-600 flex justify-between">
                          <span>Started: {maintenanceInfo.lastServiced.toLocaleDateString()}</span>
                          <span>Expected completion: {maintenanceInfo.nextDue.toLocaleDateString()}</span>
                        </div>
                    ...
                    {maintenanceInfo && (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-gray-600 space-y-1">
                          <div>
                            Target: {maintenanceInfo.nextDue.toLocaleDateString()}
                          </div>
                          <div className={maintenanceInfo.isOverdue ? "text-red-600" : "text-gray-600"}>
                            {maintenanceInfo.isOverdue 
                              ? `Overdue by ${Math.abs(maintenanceInfo.daysUntilNext)} days` 
